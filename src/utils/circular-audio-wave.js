...
export default class CircularAudioWave {
    ...
    play() {
        if (this.sourceNode && this.sourceNode.buffer) {
            ...
            this.presetOption();
            if (pausedTime) {
                startedTime = Date.now() - pausedTime;
                this.sourceNode.start(0, pausedTime/1000);
            } else {
    ...
    pause() {
        if (this.sourceNode && this.sourceNode.buffer) {
            ...
            pausedTime = Date.now() - startedTime;
            this._debouncedDraw();
        } else {
            alert('Audio is not ready');
        }
    ...
    _draw(freqData) {
        if (this.playing) {
            ...
            this.chartOption.series[1].data = waveData.data;
